...
class KreoController:
    ...
    def find_device(self):
        ...
        for d in hid.enumerate():
            ...
            # Check for matches with your specific hardware
            is_kreo = (v == self.VID_1 and p == self.PID_1) or (v == self.VID_2 and p == self.PID_2)
            
            # Look for vendor defined usage pages (0xFF00 to 0xFFFF)
            if is_kreo and up >= 0xFF00:
                print(f"Found compatible Kreo device: {d.get('product_string')} at {d.get('path')}")
                return d
        return None
